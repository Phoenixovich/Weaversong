...
async def get_sectors_and_areas():
	"""Return available sectors and areas for CityPulse filters (used by frontend dropdowns)."""
	# Try to get sectors from MongoDB first, fallback to hardcoded data
	sectors_list = []
	areas_list = list(AREA_COORDINATES.keys())
	...
	try:
		...
		async for doc in cursor:
			...
			if sector_name:
				sectors_list.append(sector_name)
	except Exception:
		# Fallback to hardcoded data if MongoDB not available
		sectors_list = list(SECTOR_BOUNDS.keys())
	
	return {"sectors": sectors_list, "areas": areas_list}
