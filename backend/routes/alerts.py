...
async def get_alerts(
    ...
    """Get all alerts, sorted by newest first. Can filter by neighborhood, category, and priority."""
    if db.database is None:
        raise HTTPException(status_code=500, detail="Database not connected")
    
    # Build query
    # Filter by neighborhood (area/sector), category, and priority
    query = {}
    if neighborhood:
...
async def analyze_alert_text(request: AlertAnalysisRequest):
    ...
    For text input, tries library-based extraction first, then falls back to AI.
    """
    if db.database is None:
        raise HTTPException(status_code=500, detail="Database not connected. Please check MongoDB connection and ensure MONGODB_URI is set in .env file.")
    
    # Initialize library_location for use later
    library_location = None
    library_title = None
    analysis = None
    
    try:
        # For speech input, ALWAYS use AI to clean and structure the transcript
        # Speech recognition often has noise, filler words, and needs proper structuring
        if request.is_speech:
            # Always use AI for speech input to clean transcript and extract structured data
            try:
                analysis = await analyze_text_with_ai(request.text, request.user_lat, request.user_lng, is_speech=True)
            except Exception as e:
                error_msg = str(e)
                if "MONGODB_URI" in error_msg or "connection" in error_msg.lower():
                    ...
                    raise HTTPException(status_code=503, detail=f"AI service error: {error_msg}. Please check API keys in .env file.")
                else:
            ...
            # Also check for library location for later use
        ...
        # Ensure we have analysis before proceeding
        if not analysis:
            raise HTTPException(status_code=500, detail="Analysis failed: No analysis result returned.")
        ...
        if location_mentions and len(location_mentions) > 0:
            ...
            if library_coords:
                ...
                # Safely get database instance without boolean evaluation
                database_instance = None
                if db.database is not None:
                    database_instance = db.database
                clustered_lat, clustered_lng = await get_clustered_coordinates(
                    location_text, base_lat, base_lng, database_instance
                )
        ...
        print(f"ERROR in analyze_alert_text ({error_type}): {error_msg}")
        print(traceback.format_exc())
        raise HTTPException(
            status_code=500,
            detail=f"Error during analysis ({error_type}): {error_msg}. Check backend terminal for full traceback."
        )
